...
SDL_Renderer *sdlk::Window::get_sdl_renderer() const { return m_sdl_renderer; }
SDL_Window *sdlk::Window::get_sdl_window() const { return m_sdl_window; }
...
sdlk::Window::Window(std::string title, sdlk::Size size,
                     sdlk::Position position, Uint32 flags) {
  m_sdl_window = SDL_CreateWindow(title.c_str(), position.m_y, position.m_y,
                                  size.m_width, size.m_height, flags);
